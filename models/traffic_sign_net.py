...
class TrafficSignNet(object):
    def __init__(self, x, y, keep_prob, keep_prob_conv, n_out=43, mu=0, sigma=0.1, learning_rate=0.001):
        self.mu = mu
        self.sigma = sigma

        self.conv1_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 1, 32),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv1_b = tf.Variable(tf.zeros(32))
        self.conv1 = (
            tf.nn.conv2d(x, self.conv1_W, strides=[1, 1, 1, 1], padding="SAME")
            + self.conv1_b
        )
        ...
        self.conv2_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 32, 32),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv2_b = tf.Variable(tf.zeros(32))
        self.conv2 = (
            tf.nn.conv2d(self.conv1, self.conv2_W, strides=[
        ...
        self.conv3_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 32, 64),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv3_b = tf.Variable(tf.zeros(64))
        self.conv3 = (
            tf.nn.conv2d(self.conv2, self.conv3_W, strides=[
                         1, 1, 1, 1], padding="SAME")
            + self.conv3_b
        )
        ...
        self.conv4_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 64, 64),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv4_b = tf.Variable(tf.zeros(64))
        self.conv4 = (
            tf.nn.conv2d(self.conv3, self.conv4_W, strides=[
                         1, 1, 1, 1], padding="SAME")
            + self.conv4_b
        )
        ...
        self.conv5_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 64, 128),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv5_b = tf.Variable(tf.zeros(128))
        self.conv5 = (
            tf.nn.conv2d(self.conv4, self.conv5_W, strides=[
                         1, 1, 1, 1], padding="SAME")
            + self.conv5_b
        )
        ...
        self.conv6_W = tf.Variable(
            tf.truncated_normal(shape=(3, 3, 128, 128),
                                mean=self.mu, stddev=self.sigma)
        )
        self.conv6_b = tf.Variable(tf.zeros(128))
        self.conv6 = (
            tf.nn.conv2d(self.conv5, self.conv6_W, strides=[
                         1, 1, 1, 1], padding="SAME")
            + self.conv6_b
        )
        ...
        self.fc1_W = tf.Variable(
            tf.truncated_normal(shape=(2048, 128),
                                mean=self.mu, stddev=self.sigma)
        )
        ...
        self.fc2_W = tf.Variable(
            tf.truncated_normal(
                shape=(128, 128), mean=self.mu, stddev=self.sigma)
        )
        ...
        self.fc3_W = tf.Variable(
            tf.truncated_normal(shape=(128, n_out),
                                mean=self.mu, stddev=self.sigma)
        )
